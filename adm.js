...
router.post('/adm/altProdutos', (req,res)=>{
	...
	//console.log(uid_usuario)
	knex('tb_produto').where({id_produto})
	.update({
		descricao: produto,
		valor: valor.replace(',','.')
	}).then(result=>{
...
router.post('/adm/remProduto', (req,res)=>{
    const { uid_usuario, id_produto } = req.body;

	console.log(uid_usuario)
	knex('tb_produto').where({id_produto: id_produto}).del().then(result=>{
		//res.redirect('/adm/produtos/'+uid_usuario);
		//res.send('Produto excluido com sucesso')
		res.status(200).send({mensagem:'Produto excluido com sucesso'});
	})
...
router.get('/adm/principal/:uid_usuario', async (req, res) => {
  try {
    const { uid_usuario } = req.params;
    ...
    /* =======================
       BUSCA USUÁRIO
    ======================= */
    const [usuario] = await knex('tb_usuario').where({ uid_usuario }).select();
    ...
    /* =======================
       BUSCA EMPRESA
    ======================= */
    const [empresa] = await knex('tb_empresa').where({ id_empresa }).select();
    ...
    /* =======================
       DATA ATUAL
    ======================= */
    const hoje = dataAtualFormatada(new Date().getDate());

    /* =======================
       CLIENTES
    ======================= */
    const cliente = await knex('tb_cliente').where({ id_empresa }).select();

    /* =======================
       LANÇAMENTOS DO DIA
    ======================= */
    const lancamentos = await knex('tb_lancamento')
      .where({
        'tb_lancamento.id_empresa': id_empresa,
        'tb_lancamento.data_externa': hoje
      })
      ...
      .select(
        'tb_lancamento.saidas',
        'tb_lancamento.id_lancamento',
        'tb_lancamento.valor',
        ...
        'tb_lancamento.data',
        'tb_lancamento.data_externa',
        'tb_lancamento.documento',
        knex.raw(`
          CASE 
            WHEN tb_lancamento.data_vencimento_externa = '1970-01-01'
            THEN '' 
            ELSE tb_lancamento.data_vencimento_externa 
          END AS data_vencimento_externa
        `),
        'tb_lancamento.observacao',
        'tb_lancamento.descricao',
        'tb_lancamento.condicao_pagamento',
        'tb_usuario.descricao as Funcionario'
      );

    /* =======================
       TOTAIS
    ======================= */
    const [[receber], [recebido]] = await Promise.all([
      knex('vwVlrTotalReceber').where({ id_empresa }).select(),
    ...
    /* =======================
       BOLETOS DASHBOARD
    ======================= */
    const boletos = await knex('tb_boletos_cobranca')
      .where({
        id_empresa,
        status: 0,
        mostrar_dashboard: 1
    ...
    const mostrar_boleto_dashboard = boletos.length ? '1' : '0';
    const link_boleto = boletos.length ? boletos[0].boleto : '';

    /* =======================
       SAÍDAS DO DIA
    ======================= */
    const [saidas] = await knex('tb_lancamento')
      .where({
        id_empresa,
        data_externa: hoje,
        saidas: 1
    ...
    /* =======================
       ENTRADAS DO DIA
    ======================= */
    const [entradasDia] = await knex('tb_lancamento as t')
      .where({ 't.data_externa': hoje })
      .whereNotIn('descricao', ['ABERTURA DE CAIXA', 'FECHAMENTO DE CAIXA'])
      .andWhere('saidas', 0)
      .sum('Valor as total');

    const [saidasDia] = await knex('tb_lancamento as t')
      .where({ 't.data_externa': hoje })
      .whereNotIn('descricao', ['ABERTURA DE CAIXA', 'FECHAMENTO DE CAIXA'])
      .andWhere('saidas', 1)
      .sum('Valor as total');
    ...
    /* =======================
       RELATÓRIO CONDIÇÃO PAGAMENTO
    ======================= */
    const relatorioCondipagto = await knex('tb_lancamento as t')
      .where({ 't.data_externa': hoje })
      .whereNotIn('descricao', ['ABERTURA DE CAIXA', 'FECHAMENTO DE CAIXA'])
      .andWhere('saidas', 0)
      .groupBy('condicao_pagamento', 'data', 'data_externa')
      .select(
    ...
    res.render('adm/principal', {
      ...
      mostrar_boleto_dashboard,
      link_boleto,

      relatorioCondipagto,
      entradasDia: entradasDia?.total || '0',
	  saidasDia: saidasDia?.total || '0',
      abrir_aviso: false
    });

  } catch (error) {
    console.error('Erro na rota /adm/principal:', error);
...
router.get('/adm/principal/:uid_usuario', (req,res)=>{
    ...
    knex('tb_usuario').where({uid_usuario: uid_usuario}).select().then(result=>{		
		...
		knex('tb_empresa').where({id_empresa: id_empresa}).select().then(result_empresa=>{			
			...
			.leftJoin('tb_cliente','tb_cliente.id_cliente','tb_lancamento.id_cliente')			
			.leftJoin('tb_usuario','tb_usuario.uid_usuario','tb_lancamento.uid_usuario')			
			.select('tb_lancamento.saidas', 'tb_lancamento.id_lancamento','tb_lancamento.valor','tb_lancamento.data_pagamento_externa',
						 ...
						 knex('vwVlrTotalRecebido').where({id_empresa: id_empresa}).select().then(vlrTotalRecebido=>{
							...
							knex('tb_boletos_cobranca').where({id_empresa: id_empresa}).andWhere({status: 0}).andWhere({mostrar_dashboard: 1}).select().then(mostrar_boleto=>{
								...
								.select().then(saidas=>{
									...
									knex('tb_lancamento as t').select(
											knex.raw('SUM(Valor) as total'))
									.where({'t.data_externa': dataDoDia})
									.whereNotIn('descricao', ['ABERTURA DE CAIXA','FECHAMENTO DE CAIXA'])
									.andWhere('saidas', 0)
									.then(entradasDia =>{
										knex('tb_lancamento as t').select(
										...
										.whereNotIn('descricao', ['ABERTURA DE CAIXA','FECHAMENTO DE CAIXA'])
										.andWhere('saidas', 0)
										.groupBy('condicao_pagamento', 'data','data_externa').then(relatorioCondipagto =>{
											...
											relatorioCondipagto,
											entradasDia: entradasDia[0].total
										})	
								  })
    ...
    knex('tb_usuario').where({uid_usuario: uid_usuario}).select().then(result=>{
		...
		knex('tb_cliente').where({id_empresa: id_empresa}).select().then(cliente=>{
			...
			knex('tb_lancamento').where({'tb_lancamento.id_empresa': id_empresa})
				.leftJoin('tb_cliente','tb_cliente.id_cliente','tb_lancamento.id_cliente')
				.select('tb_lancamento.id_lancamento','tb_lancamento.documento','condicao_pagamento',
						'tb_lancamento.valor',
						'tb_cliente.nome',
						'tb_lancamento.status',
						'tb_lancamento.data',
...
router.get('/adm/usuarios/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;

    knex('tb_usuario').select().then(result=>{		
		console.log(result)
